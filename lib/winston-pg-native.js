...
class Postgres extends winston.Transport {
  constructor(options = {}) {
    ...
    let tableFields = tableConfig.tableFields || options.tableFields || ['level', 'message', 'meta'];
  ...
  log(...args) {
    ...
    const meta = args.shift();
    const callback = args.shift();

    const logger = this;
    const pool = this.pool;
    const sqlStatement = this.sqlStatement;
    ...
    return pool.connect()
      .then(client => client.query({
        text: sqlStatement,
        values: ['now()', level, message, circularJSON.stringify(meta)],
      }).then(() => {
        client.release();
        logger.emit('logged');
        return callback(null, true);
